...
- 본문(한국어): 전체 열 줄 안팎, 아래 세 부분만.
  ...
  1. **무엇을·왜** — 1~2줄.
  2. **봐 주세요** — 리뷰어가 실제로 읽어야 할 1~3곳. **한두 줄 설명이 본체다** — 무엇이
     걸리는지(위험한 이유 · 갈린 판단 · 확신 없는 부분)를 본문에서 바로 알 수 있게 쓴다.
     **포인터만 던지지 않는다**: 읽는 사람이 건별로 파일을 찾아 열어야 하면 그냥 안 읽는다.
     코드를 직접 봐야 이해되는 건이면 `경로:심볼` 대신 **permalink** 를 건다 — GitHub 이
