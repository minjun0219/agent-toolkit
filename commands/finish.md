...
- 본문(한국어): 전체 열 줄 안팎, 아래 세 부분만.
  ...
  1. **무엇을·왜** — 1~2줄.
  2. **봐 주세요** — 리뷰어가 실제로 읽어야 할 1~3곳. 각 항목은 `경로:심볼` **+ 한두 줄
     설명**이다 — 무엇이 걸리는지(위험한 이유 · 갈린 판단 · 확신 없는 부분)를 본문에서
     바로 알 수 있게 쓴다. **포인터만 던지지 않는다**: 읽는 사람이 건별로 파일을 찾아
     열어야 하면 그냥 안 읽는다. 없으면 `없음` 이라고 쓴다 — 억지로 만들지 않는다.
